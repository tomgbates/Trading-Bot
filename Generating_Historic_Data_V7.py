...
def choice_years():
    f = open('./Historic_Data_csv/Details.txt', 'a')
...
while j <= FILE_NUMBER:
    ...
    while i <= iteration_total:
        ...
        if up_or_down_decision == 'up':
            ...
            f = open('./Historic_Data_csv/' + CSV_FILE_WRITE, 'a')
            ...
            i += 1
    end_time = time.time()
    iteration_time = '%.2f' % float(end_time - start_time)
    f = open('./Historic_Data_csv/Details.txt', 'a')
